...
def getDrugMolecularGraph(ligands):
    #接受一个字典 ligands 作为输入，其中键是药物标识符，值是相应配体的SMILES表示法。
    # create molecule graph
    print("create molecule graph ...")
    #创建了一个有序字典（OrderedDict）对象smile_graph 被用来存储药物的标识符和对应的 SMILES 表示法转换成的分子图。
...
#函数的目的是通过选择每行中最大的前 k 个元素，创建一个新的矩阵 refine_adj。
def denseAffinityRefine(adj, k):
    #创建一个与输入矩阵 adj 相同形状的全零矩阵，命名为 refine_adj
    refine_adj = np.zeros_like(adj)
    #创建一个二维数组 indexs1，其中包含 0 到 adj.shape[0] - 1 的整数，
    #然后使用 np.tile 将其复制为 k 行，最后使用 transpose 进行转置。这用于创建一个矩阵，其中每行包含相同的行索引。
    indexs1 = np.tile(np.expand_dims(np.arange(adj.shape[0]), 0), (k, 1)).transpose()
    #使用 np.argpartition 对矩阵 adj 进行部分排序，选择每行最大的前 k 个元素的列索引。
    # 这样得到的 indexs2 是一个包含最大元素列索引的矩阵。
    indexs2 = np.argpartition(adj, -k, 1)[:, -k:]
    #将 adj 中对应于 indexs1 和 indexs2 的位置上的元素赋值给 refine_adj。
    # 这样，refine_adj 中每行只保留了每行中最大的前 k 个元素，其余元素为零。
...
def getAffinityGraph(dataset, adj, weighted, drug_aff_k, target_aff_k):
    ...
    # 根据数据集类型进行不同的处理 论文中有介绍
    if dataset == "davis":
        # 如果是"davis"数据集，对非零元素减去5，并进行最小-最大归一化
        adj[adj != 0] -= 5
        adj_norm = minMaxNormalize(adj, 0)
    elif dataset == "kiba":
        # 如果是"kiba"数据集，先对转置后的亲和矩阵进行密集亲和性细化
