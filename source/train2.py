...
def create_dataset_for_train_test(affinity, dataset, fold, weighted, drug_aff_k, target_aff_k, drug_sim_k):
    ...
    drug_mask_fold = json.load(open(dataset_path + 'S2_mask_set.txt')) if fold == -100 else json.load(
        open(dataset_path + 'S2_test_set.txt')) + json.load(open(dataset_path + 'S2_mask_set.txt'))
...
def train_test():
    ...
    for epoch in range(NUM_EPOCHS):
        train(architecture, predictor, device, train_loader, train_drug_graphs_DataLoader, target_graphs_DataLoader, LR,
        ...
        G, P = predicting(architecture, predictor, device, test_loader, test_drug_graphs_DataLoader,
                          target_graphs_DataLoader, affinity_graph, drug_map=drug_test_map,
