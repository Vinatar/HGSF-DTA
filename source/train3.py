...
def create_dataset_for_train_test(affinity, dataset, fold, weighted, drug_aff_k, target_aff_k, target_sim_k):
    ...
    target_test_fold = json.load(open(dataset_path + 'S3_test_set.txt')) if fold == -100 else target_train_fold_origin[
        fold]
    target_mask_fold = json.load(open(dataset_path + 'S3_mask_set.txt')) if fold == -100 else json.load(
        open(dataset_path + 'S3_test_set.txt')) + json.load(open(dataset_path + 'S3_mask_set.txt'))
...
def train_test():
    ...
    train_data, test_data, affinity_graph, drug_graphs_dict, train_target_graphs_dict, test_target_graphs_dict, target_count, target_test_map, target_test_map_weight_norm = \
        create_dataset_for_train_test(affinity, dataset, fold, FLAGS.weighted, FLAGS.drug_aff_k, FLAGS.target_aff_k,
                                      FLAGS.target_sim_k)
    ...
    drug_graphs_DataLoader = torch.utils.data.DataLoader(drug_graphs_Data, shuffle=False, collate_fn=collate,
                                                         batch_size=affinity_graph.num_node1s)
    ...
    for epoch in range(NUM_EPOCHS):
        train(architecture, predictor, device, train_loader, drug_graphs_DataLoader, train_target_graphs_DataLoader, LR,
              epoch + 1, TRAIN_BATCH_SIZE, affinity_graph)
        G, P = predicting(architecture, predictor, device, test_loader, drug_graphs_DataLoader,
                          test_target_graphs_DataLoader, affinity_graph, target_map=target_test_map,
