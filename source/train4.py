...
from preprocessing import process_drug_data, process_target_data
from myutils import argparser, DTADataset, GraphDataset, collate, predicting, read_data, train
...
def create_dataset_for_train_test(affinity, dataset, weighted, drug_aff_k, target_aff_k, drug_sim_k, target_sim_k):
    ...
    train_dataset = DTADataset(drug_ids=train_rows, target_ids=train_cols, drug_smiles=drug_list,
                               target_sequences=target_list, y=train_Y)

    test_rows, test_cols = np.where(np.isnan(test_affinity) == False)
    test_Y = test_affinity[test_rows, test_cols]
    test_dataset = DTADataset(drug_ids=test_rows, target_ids=test_cols, drug_smiles=drug_list,
                              target_sequences=target_list, y=test_Y)
    ...
    target_test_map_weight = target_test_train_sim[np.tile(np.expand_dims(np.arange(target_test_train_sim.shape[0]), 0),
                                                           (target_sim_k, 1)).transpose(), target_test_train_map]
...
def train_test():
    ...
    train_data, test_data, affinity_graph, \
        train_drug_graphs_dict, test_drug_graphs_dict, train_target_graphs_dict, test_target_graphs_dict, drug_count, target_count, \
        drug_test_map, drug_test_map_weight_norm, target_test_map, target_test_map_weight_norm = \
        create_dataset_for_train_test(affinity, dataset, FLAGS.weighted, FLAGS.drug_aff_k, FLAGS.target_aff_k,
                                      FLAGS.drug_sim_k, FLAGS.target_sim_k)
    ...
    train_drug_graphs_DataLoader = torch.utils.data.DataLoader(train_drug_graphs_Data, shuffle=False,
                                                               collate_fn=collate, batch_size=affinity_graph.num_node1s)
    test_drug_graphs_Data = GraphDataset(graphs_dict=test_drug_graphs_dict, dttype="drug")
    test_drug_graphs_DataLoader = torch.utils.data.DataLoader(test_drug_graphs_Data, shuffle=False, collate_fn=collate,
                                                              batch_size=drug_count - affinity_graph.num_node1s)
    train_target_graphs_Data = GraphDataset(graphs_dict=train_target_graphs_dict, dttype="target")
    train_target_graphs_DataLoader = torch.utils.data.DataLoader(train_target_graphs_Data, shuffle=False,
                                                                 collate_fn=collate,
                                                                 batch_size=affinity_graph.num_node2s)
    test_target_graphs_Data = GraphDataset(graphs_dict=test_target_graphs_dict, dttype="target")
    test_target_graphs_DataLoader = torch.utils.data.DataLoader(test_target_graphs_Data, shuffle=False,
                                                                collate_fn=collate,
                                                                batch_size=target_count - affinity_graph.num_node2s)
    ...
    for epoch in range(NUM_EPOCHS):
        train(architecture, predictor, device, train_loader, train_drug_graphs_DataLoader,
              train_target_graphs_DataLoader, LR, epoch + 1, TRAIN_BATCH_SIZE, affinity_graph)
        G, P = predicting(architecture, predictor, device, test_loader, test_drug_graphs_DataLoader,
                          test_target_graphs_DataLoader, affinity_graph, drug_map=drug_test_map,
                          drug_map_weight=torch.FloatTensor(drug_test_map_weight_norm).to(device),
                          target_map=target_test_map,
                          target_map_weight=torch.FloatTensor(target_test_map_weight_norm).to(device))
